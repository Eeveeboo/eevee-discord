  ...
  static bindAllMethods(scope:{}) {
    for (var p in scope) {
      if(scope.hasOwnProperty(p)) {
        ...
        if (descriptor && (descriptor.get || descriptor.set)) continue; // Don't bind if `scope[p]` is a getter/setter, we'd be attemping to bind the value returned by the getter
        ...
        if (
          !Object.prototype.hasOwnProperty.call(scope, p) &&
          typeof (<any>scope)[p] == "function"
        ) {
          console.log("Binding", p);
          (<any>scope)[p] = (<any>scope)[p].bind(scope);
        }
      }else{
